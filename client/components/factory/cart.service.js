'use strict';
  ...
  function ShoppingCart(cartName, Settings, Shipping, Order, Mail, Pay, appConfig, $state) {
    ...
    // items in the cart
    //
    function CartItem(sku, name, slug, mrp, price, quantity, image, category, size, weight, vid) {
        this.sku = sku;
        this.name = name;
        this.slug = slug;
        this.image = image;
  ...
  ShoppingCart.prototype.addItem = function (product,quantity) {
      ...
      if (quantity !== 0) {
          ...
          if (!found && product.price) {
              var itm = new CartItem(product.sku, product.name, product.slug, product.mrp, product.price, product.quantity, product.image, product.category, product.size, product.weight, product.vid);
              this.items.push(itm);
...
ShoppingCart.prototype.checkoutPayPal = function (parms, clearCart) {
    ...
    var options = {uid: opt.uid, email: opt.email, recipient_name: opt.name, phone: opt.phone, line1: opt.address, city: opt.city, postal_code:opt.zip, country_code: opt.country_code, discount: opt.couponAmount, shipping: opt.shipping.charge, currency_code:opt.currency_code, exchange_rate:opt.exchange_rate}
    var data = {
        cmd: "_cart",
...
ShoppingCart.prototype.checkoutCOD = function (parms, clearCart) {
    ...
    var orderDetails = {
        ...
        amount: {total: total, currency: opt.currency_code, details:{shipping:Math.round(opt.shipping.charge*100)/100,subtotal:subtotal}},
