...
export function loaded(req, res) {
  return Category.find({parent: null}).
  populate({path: 'child',match: {products: { $exists: true, $ne: [] } }, populate: ({ path: 'child',populate: ({ path: 'child', populate: ({ path: 'child', populate: ({ path: 'child', populate: ({ path: 'child',populate: ({ path: 'child',populate: ({ path: 'child',populate: ({ path: 'child' }) }) }) }) }) }) }) })
  }).sort('-name').exec()
    .then(respondWithResult(res))
    .catch(handleError(res));
}
